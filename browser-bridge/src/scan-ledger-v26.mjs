...
export function buildScanLedgerEntry({id, finalStatus, startedAt, completedAt=Date.now(), request, latestLive, connected, contentStatus, postStatus}) {
  const scanSignals=buildScanSignals(latestLive.evidence||[],latestLive.inferredTopics||[],24);
  ...
  return {
      ...
      transcriptEvidence: latestLive.evidence.filter(row=>clean(row.transcript,40)).length,
