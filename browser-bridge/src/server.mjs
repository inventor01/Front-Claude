...
function queueEvidence(rows) {
  pendingEvidence = dedupeEvidence([...pendingEvidence, ...rows]).slice(-500);
...
async function scrollViewport(page, passes, pauseMs = 900) {
  for (let i = 0; i < passes; i++) {
    await page.evaluate(() => window.scrollBy(0, Math.max(window.innerHeight * 0.9, 700))).catch(() => {});
...
async function collectXQuery(query, limit) {
  ...
  try {
    ...
    await scrollViewport(page, 1, 700);
    const out = await extractXArticles(page, limit, `Local Chrome browser · X Latest search · query: ${q}`);
...
async function collectXHome(limit, passes) {
  ...
  try {
    ...
    const out = [];
    for (let i = 0; i <= passes; i++) {
      out.push(...await extractXArticles(page, limit, 'Local Chrome browser · X Home feed sample'));
      if (dedupeEvidence(out).length >= limit || i === passes) break;
      await scrollViewport(page, 1, 850);
    }
    const unique = dedupeEvidence(out).slice(0, limit);
    if (!unique.length) throw new Error('X Home loaded but no posts were extractable. Confirm the Front Chrome session is signed in and Home is available.');
    return unique;
  } finally { await page.close(); }
}
...
async function collectXExplore(limit) {
  ...
  try {
    ...
    for (let i = 0; i < count && out.length < limit; i++) {
      const item = candidates.nth(i); const raw = (await item.innerText().catch(() => '')).trim(); const text = xTrendLabel(raw); if (!text) continue;
...
async function extractTikTokPage(page, provenance, limit) {
  ...
  const anchorCount = Math.min(await anchors.count(), limit * 3);
...
async function collectTikTokSearch(topic, limit) {
  const q = sanitizeTopic(topic); if (!q) return [];
  const page = await newPage(`https://www.tiktok.com/search?q=${encodeURIComponent(q)}`);
  try {
    await page.waitForTimeout(2800); await scrollViewport(page, 1, 800);
    const out = await extractTikTokPage(page, `Local Chrome browser · TikTok search · query: ${q}`, limit);
...
async function collectTikTokExplore(limit, passes = 2) {
  const page = await newPage('https://www.tiktok.com/explore');
  try {
    await page.waitForTimeout(3000);
    const out = [];
    for (let i = 0; i <= passes; i++) {
      out.push(...await extractTikTokPage(page, 'Local Chrome browser · TikTok Explore feed sample', limit));
      if (dedupeEvidence(out).length >= limit || i === passes) break;
      await scrollViewport(page, 1, 900);
    }
    const unique = dedupeEvidence(out).slice(0, limit);
    if (!unique.length) throw new Error('TikTok Explore loaded but no extractable videos were found.');
    return unique;
  } finally { await page.close(); }
}
...
async function collectTikTokTrends(limit) {
  ...
  try {
    ...
    try {
      ...
      const count = Math.min(await rows.count(), Math.max(limit * 3, limit)); const out = [];
      for (let i = 0; i < count && out.length < limit; i++) {
        const row = rows.nth(i); const text = (await row.innerText().catch(() => '')).replace(/\n+/g,' · ').trim(); const href = await row.getAttribute('href').catch(() => null);
        if (!text || !href) continue; const url = href.startsWith('http') ? href : `https://ads.tiktok.com${href}`;
      ...
      if (!out.length) {
        const body = await page.locator('body').innerText().catch(() => '');
        for (const tag of extractHashtags(body, limit)) { const url=`https://www.tiktok.com/search?q=${encodeURIComponent(tag)}`; out.push({id:stableId('TikTok',url,tag),platform:'TikTok',author:'TikTok Creative Center',url,content:`#${tag}`,published:null,views:null,likes:null,provenance:'Local Chrome browser · TikTok Creative Center hashtag fallback'}); }
      }
      if (out.length) return out;
      creativeError = new Error('Creative Center loaded but no trend evidence was extractable.');
    } finally { await page.close(); }
  } catch (error) { creativeError = error; }
  try { return await collectTikTokExplore(limit, 1); }
  catch (fallbackError) {
...
async function collectAll(input = {}) {
  ...
  try {
    const active = normalizeConfig({...config,...input}); const evidence=[]; const errors=[];
    if (active.scanXExplore) try { evidence.push(...await collectXExplore(active.resultsPerQuery)); } catch(e){errors.push(`X Explore: ${e.message}`);}
    if (active.scanXHome) try { evidence.push(...await collectXHome(active.maxFeedItems,active.scrollPasses)); } catch(e){errors.push(`X Home sample: ${e.message}`);}
    for (const account of active.xAccounts.slice(0,25)) try { evidence.push(...await collectXQuery(`from:${account} -filter:replies`,active.resultsPerQuery)); } catch(e){errors.push(`X @${account}: ${e.message}`);}
    for (const keyword of active.keywords.slice(0,20)) try { evidence.push(...await collectXQuery(`"${keyword}" -filter:replies`,active.resultsPerQuery)); } catch(e){errors.push(`X ${keyword}: ${e.message}`);}
    if (active.scanTikTokTrends) {
      try {
        const trends=await collectTikTokTrends(active.resultsPerQuery*2); evidence.push(...trends);
        const topics=trends.map((x)=>sanitizeTopic(x.content.split('·')[0])).filter(Boolean).slice(0,active.maxTrendQueries);
        for(const topic of topics) try{evidence.push(...await collectTikTokSearch(topic,active.resultsPerQuery));}catch(e){errors.push(`TikTok ${topic}: ${e.message}`);}
      } catch(e){errors.push(`TikTok trends: ${e.message}`);}
    }
    if (active.scanTikTokExplore) try { evidence.push(...await collectTikTokExplore(active.maxFeedItems,active.scrollPasses)); } catch(e){errors.push(`TikTok Explore sample: ${e.message}`);}
    for(const keyword of active.keywords.slice(0,active.maxTrendQueries)) try{evidence.push(...await collectTikTokSearch(keyword,active.resultsPerQuery));}catch(e){errors.push(`TikTok ${keyword}: ${e.message}`);}

    let topics = inferTopics(evidence, Date.now(), active.inferredTopicSearches);
    for (const topic of topics.slice(0,active.inferredTopicSearches)) {
      try { evidence.push(...await collectXQuery(`"${topic.topic}" -filter:replies`,active.resultsPerQuery)); } catch(e){errors.push(`X inferred ${topic.topic}: ${e.message}`);}
      try { evidence.push(...await collectTikTokSearch(topic.topic,active.resultsPerQuery)); } catch(e){errors.push(`TikTok inferred ${topic.topic}: ${e.message}`);}
    }
    const result=dedupeEvidence(evidence); topics=inferTopics(result,Date.now(),10);
    lastRun=Date.now(); lastCount=result.length; lastTopics=topics; lastError=errors.length?errors.join(' | '):null;
    return {evidence:result,errors,inferredTopics:topics,at:lastRun,config:active};
  } finally { running=false; }
}

function scheduleNext() {
  clearTimeout(scheduleTimer); nextScheduledRun=null;
  if (!config.enabled) return;
  const delay=config.intervalMinutes*60000; nextScheduledRun=Date.now()+delay;
  scheduleTimer=setTimeout(async()=>{
    nextScheduledRun=null;
    try { const result=await collectAll(config); queueEvidence(result.evidence); }
...
const server=http.createServer(async(req,res)=>{
  ...
  try{
    ...
    if(req.method==='GET'&&url.pathname==='/pending')return json(req,res,200,{evidence:pendingEvidence.slice(-250),count:pendingEvidence.length,lastRun,lastTopics});
    if(req.method==='POST'&&url.pathname==='/ack') { let body='';for await(const chunk of req)body+=chunk;const parsed=JSON.parse(body||'{}');return json(req,res,200,{ok:true,removed:ackEvidence(parsed.ids),remaining:pendingEvidence.length}); }
    if(req.method==='POST'&&url.pathname==='/config'){let body='';for await(const chunk of req)body+=chunk;config=normalizeConfig(JSON.parse(body||'{}'));writeJson(configPath,config);scheduleNext();return json(req,res,200,{ok:true,config});}
    if(req.method==='POST'&&url.pathname==='/scan'){let body='';for await(const chunk of req)body+=chunk;return json(req,res,200,await collectAll(JSON.parse(body||'{}')));}
    if(req.method==='POST'&&url.pathname==='/open-login'){
      if(!systemChrome)throw new Error('Google Chrome is required for authenticated X/TikTok scans. Install Chrome, then restart the Front browser bridge.');
      if(browserConnection?.isConnected?.())await browserConnection.close().catch(()=>{});browserConnection=undefined;context=undefined;
      stopExistingFrontChrome({dataDir});await new Promise((resolve)=>setTimeout(resolve,700));
      const opened=openRegularChromeForLogin({dataDir,chromeExecutable:systemChrome,debuggingPort:cdpPort});
      try{await waitForCdp(opened.cdpUrl,{timeoutMs:12000});}catch(error){throw new Error(`Front Chrome opened, but its local scan connection did not start. Close the Front Chrome window and click Open X + TikTok login once more. ${error instanceof Error?error.message:String(error)}`);}
      return json(req,res,200,{ok:true,message:'Front Chrome is ready. Sign in to X and TikTok and leave this Front Chrome window open or minimized. Manual and scheduled scans can now use it.',profileDir:opened.profileDir,cdpUrl:opened.cdpUrl});
    }
    return json(req,res,404,{error:'Not found'});
  }catch(error){lastError=error instanceof Error?error.message:String(error);return json(req,res,500,{error:lastError});}
});
...
server.listen(PORT,HOST,()=>{
  ...
  console.log(`[front-bridge] scheduled scan: every ${config.intervalMinutes} minute(s) while enabled; pending evidence stays local until Front syncs it`);
