...
async function collectAll(input = {}) {
  ...
  try {
    ...
    const initialEvidence = mergeRichEvidence(evidence);
    let firstPass = detectTopics(initialEvidence, firstAt, Math.max(active.inferredTopicSearches, 8));
    firstPass = attachSoundSignals(firstPass, initialEvidence);
    firstPass = attachVisualSignals(firstPass, initialEvidence);
    firstPass = semanticConsolidateTopics(firstPass, initialEvidence);
    ...
    if (scanMode === 'deep' && firstPass.length) originSearches = await runOriginResearch(firstPass, active, add, errors);
    ...
    topics = attachVisualSignals(topics, result);
    topics = semanticConsolidateTopics(topics, result);
...
function status() {
  return {
    ...
    cdpUrl,
    capabilities: [
      'x-for-you','tiktok-for-you','adaptive-scroll','sentinel-rotation','candidate-investigation','feed-penetration',
      'sound-signals','source-reputation','alias-consolidation','semantic-context-clustering','visual-perceptual-hash',
...
server.listen(PORT, HOST, () => {
  console.log(`Front browser bridge v11 listening on http://${HOST}:${PORT}`);
  console.log('Discovery: For You scout → semantic/media clustering → automatic Deep escalation → targeted origin research → momentum + launch watch.');
  scheduleNext();
});
