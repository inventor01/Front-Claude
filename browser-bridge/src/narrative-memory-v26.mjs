...
export function summarizeNarrativeMemory(record,now=Date.now()){
  ...
  let weightedSupport=0,recent6h=0,prior18h=0;
  for(const item of active){
    const age=Math.max(0,now-Number(item.at||0));
    const weight=narrativeMemoryWeight(age);
    weightedSupport+=weight;
    if(item.creator)creatorWeights.set(item.creator,Math.max(creatorWeights.get(item.creator)||0,weight));
  ...
  return{
    ...
    creatorCount:creatorWeights.size,
...
export function memoryScanSignals(memoryInput,{now=Date.now(),currentEvidenceIds=[]}={}){
  ...
  for(const [key,record] of Object.entries(memory.narratives)){
    ...
    if(summary.creatorCount>=3&&summary.weightedCreators>=2.1&&(summary.platforms.length>=2||summary.acceleration>.5))scanStatus='RISING';
    else if(summary.creatorCount>=2&&summary.weightedCreators>=1.4)scanStatus='EARLY';
    ...
    out.push({topic:clean(record.title,140)||key,key,tier:'pre-breakout',corroborated:false,evidenceCount:Math.max(evidenceIds.length,summary.evidenceCount),authorCount:summary.creatorCount,platforms:summary.platforms,evidenceIds,score:signalScore,signalScore,scanStatus,signalSource:'memory',memory:summary,aliases:record.aliases||[]});
  }
