...
export function buildScanSignals(evidence=[],inferredTopics=[],limit=24){
  ...
  for(const topic of inferredTopics){
    const key=topicKey(topic);if(!key)continue;
    const signal={...topic,key,signalScore:clamp(Math.round(Number(topic.score||0))),signalSource:'engine'};
  ...
  for(const row of evidence){
    ...
    const subject=clean(row.postSubject||row.postEvent,120);
    if(!key||confidence<.5||terms(subject).length<1)continue;
  ...
  for(const [key,rows] of semantic){
    ...
    const title=clean(rows.map((row)=>row.postSubject||row.postEvent).find((value)=>terms(value).length)||key,120);
