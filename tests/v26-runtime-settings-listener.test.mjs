...
test('local vision defaults match the proven 8 GB Mac profile and do not cap discovery',()=>{
  ...
  assert.match(start,/replacing stale deep visual budget 8 with balanced budget 2/);
  assert.match(start,/replacing stale scout visual budget 4 with balanced budget 1/);
  assert.match(start,/replacing stale background visual budget 2 with balanced budget 1/);
  assert.match(start,/replacing stale contextual post budget 90 with balanced budget 12/);
