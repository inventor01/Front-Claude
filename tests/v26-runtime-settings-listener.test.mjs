...
test('local vision defaults match the proven 8 GB Mac profile and do not cap discovery',()=>{
  ...
  assert.match(env,/FRONT_CONTENT_TIMEOUT_MS=75000/);
  assert.match(env,/FRONT_CONTENT_SCOUT_TIMEOUT_MS=45000/);
  assert.match(env,/FRONT_CONTENT_NUM_PREDICT=420/);
  assert.match(env,/FRONT_CONTENT_DEEP_VIDEOS=2/);
  assert.match(env,/FRONT_CONTENT_SCOUT_VIDEOS=1/);
  ...
  assert.match(start,/FRONT_CONTENT_TIMEOUT_MS:-75000/);
  assert.match(start,/FRONT_CONTENT_SCOUT_TIMEOUT_MS:-45000/);
  assert.match(start,/FRONT_CONTENT_NUM_PREDICT:-420/);
  assert.match(start,/FRONT_CONTENT_DEEP_VIDEOS:-2/);
  assert.match(start,/FRONT_CONTENT_SCOUT_VIDEOS:-1/);
  ...
  assert.match(start,/replacing stale deep visual timeout 60000ms with measured-safe 75000ms/);
  assert.match(start,/replacing stale deep visual budget \$FRONT_CONTENT_DEEP_VIDEOS with reliable budget 2/);
  assert.match(start,/\[ "\$FRONT_CONTENT_DEEP_VIDEOS" = "8" \].*\[ "\$FRONT_CONTENT_DEEP_VIDEOS" = "4" \]/s);
  assert.match(start,/replacing stale scout visual budget \$FRONT_CONTENT_SCOUT_VIDEOS with reliable budget 1/);
  assert.match(start,/\[ "\$FRONT_CONTENT_SCOUT_VIDEOS" = "4" \].*\[ "\$FRONT_CONTENT_SCOUT_VIDEOS" = "2" \]/s);
