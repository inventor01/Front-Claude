...
## V27 clean-disk release diagnosis — 2026-09-15

The authenticated baseline at `00f13be` started with 7.15 GiB free, collected 37 X posts and 69 TikTok rows, and requested 79 video transcripts. Transcription finished with 60 complete / 19 failed / 0 unavailable: ten X fragment failures and nine TikTok feed-ad failures. These reproduce without disk exhaustion. Visual capture succeeded in 4.7s but local inference exceeded 61s. Full baseline outcome and corrected-head release validation remain pending.

- **Incomplete media selected as files:** X HLS `.m4s` segments served as `video/mp4` outranked their playlist and filled the 12-candidate limit. Exclude fragments from standalone acquisition. A previously failing X post now decodes audio using the retained master playlist in an authenticated replay.
- **CDN scripts treated as media:** TikTok's host-name score admitted JavaScript bundles; a live page's twelve selected candidates were all scripts. Reject non-media content types and static-asset extensions before ranking.
- **Feed-only promotional media lost:** all nine TikTok failures had “Discover more today!” captions. Replay returned “Video currently unavailable”; an authenticated feed response identifies these as ads and supplies playable media URLs. Retain video-ID/author-scoped feed media through observation and acquire it directly. Keep these videos in coverage, including a decoded frame for silent-video meaning.
- **False no-speech path:** zero browser capture tracks were accepted without media acquisition. Require successful Whisper or a precise decoder audio-stream absence result; invalidate browser-track-only cached no-speech. Invalid bytes, HTTP failures and codec failures remain failures. Actual ffmpeg fixtures cover audio, video without audio, and invalid input.
- **Resource retention and missing diagnostics:** dispose Playwright API response bodies after copying media; retain sanitized per-candidate type/status/size/decoder diagnostics for analysis.
- **Semantic batch latency and failure propagation:** observed eight-row local batch hit 60s after processing 2,578 input tokens and generating only 224 output tokens. Bound local batches by complete input size and at most four rows. Retry splits isolate failed single rows rather than aborting successful siblings. Weak meanings remain failures. No confidence, coverage or release requirement is relaxed.
- **Release naming:** the v27 wrapper imported a validator with hard-coded v26 human-facing labels. Update shared gate/launcher labels while preserving compatibility filenames, ledger paths and protocol version.

Regression checks for these changes pass locally. Full automated QA, exact-head CI and a fresh authenticated release gate are required before merge/deployment.

Baseline validator ended after 20 minutes with 65/79 meanings and 14 meaning failures; it timed out during post-understanding before ranking. The bridge subsequently reached failed terminal state. A real silent X replay proved frame readiness=4 and a cross-origin canvas `SecurityError`. The meaning fallback now uses a video-element screenshot only after media readiness; a real cross-origin Chromium fixture verifies cropped red-frame pixels and bounded dimensions. A live feed-ad acquisition also returned valid audio and a decoded frame (HTTP 200), confirming the retained feed URL path. X's `/aud|vid/<codec>/0/0/` MP4 URLs were measured as 786–904 byte initialization headers and are also excluded. Visible-caption snippets without explicit completion status no longer bypass full transcription.
