...
function words(value:string){return normalize(value).split(' ').filter((token)=>token.length>=3&&!STOP.has(token)&&!GENERIC.has(token)&&!BROAD.has(token)&&!/\d/.test(token));}
