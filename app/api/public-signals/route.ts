...
type CandidateLabel={key:string;label:string;kind:'hashtag'|'name'|'token'|'phrase'};
...
function slugTopic(url:string){try{const u=new URL(url);const parts=u.pathname.split('/').filter(Boolean);const index=parts.findIndex((part)=>part==='hashtag'||part==='trend'||part==='trends');if(index>=0&&parts[index+1]){const decoded=decodeURIComponent(parts[index+1]).replace(/[-_]+/g,' ').trim();return decoded;}}catch{}return '';}
...
function words(value:string){return normalize(value).split(' ').filter((token)=>token.length>=3&&!STOP.has(token)&&!GENERIC.has(token)&&!BROAD.has(token)&&!COMMON_SINGLE.has(token)&&!/\d/.test(token));}
function candidateLabels(row:BrowserRow):CandidateLabel[]{
  const out=new Map<string,CandidateLabel>(); const add=(raw:string,kind:CandidateLabel['kind'])=>{const label=cleanCandidate(raw).slice(0,80),key=normalize(label),tokens=words(label);if(!key||isJunkLabel(label,row.author)||!tokens.length)return;if(!out.has(key))out.set(key,{key,label,kind});};
  ...
  const normalizedWords=words(row.content).slice(0,60);for(const token of normalizedWords)if(token.length>=4&&token.length<=30)add(token,'token');
  for(let size=2;size<=3;size++)for(let i=0;i<=normalizedWords.length-size;i++)add(normalizedWords.slice(i,i+size).join(' '),'phrase');
  return[...out.values()];
}
function buildEarlyCandidates(rows:BrowserRow[],linked:Set<string>,now:number):PublicItem[]{
  const groups=new Map<string,CandidateGroup>();
  for(const row of rows){if(linked.has(row.id)||row.last_seen<now-36*3600000)continue;for(const candidate of candidateLabels(row)){const group=groups.get(candidate.key)||{labels:new Map<string,number>(),rows:[],authors:new Set<string>(),platforms:new Set<string>(),kinds:new Set<CandidateLabel['kind']>()};if(!group.rows.some((item)=>item.id===row.id))group.rows.push(row);group.authors.add(`${row.platform}:${row.author.toLowerCase()}`);group.platforms.add(row.platform);group.kinds.add(candidate.kind);group.labels.set(candidate.label,(group.labels.get(candidate.label)||0)+1);groups.set(candidate.key,group);}}
  return[...groups.entries()].flatMap(([key,group])=>{const label=[...group.labels.entries()].sort((a,b)=>b[1]-a[1]||a[0].length-b[0].length)[0]?.[0]||key;const labelWords=normalize(label).split(' ').filter(Boolean);const structured=group.kinds.has('hashtag')||(group.kinds.has('name')&&labelWords.length>=2);const cross=group.platforms.size>1;const enough=structured?group.authors.size>=2:group.authors.size>=3||(cross&&group.authors.size>=2);if(!enough||!words(label).length)return[];const top=[...group.rows].sort((a,b)=>((b.views||0)+(b.likes||0)*4)-((a.views||0)+(a.likes||0)*4)||b.last_seen-a.last_seen)[0];if(isJunkLabel(label))return[];const latest=Math.max(...group.rows.map((row)=>row.last_seen));const published=group.rows.map((row)=>row.published).filter((v):v is number=>typeof v==='number'&&v>0);const engagement=Math.log10(1+(top.views||0))+0.35*Math.log10(1+(top.likes||0));const quality=group.authors.size*4+group.platforms.size*3+(structured?3:0)+Math.min(5,engagement);return[{id:`candidate:${key}`,source:'Early Candidate',sourceKey:'early-candidate',title:label,query:label,url:top.url,observed:latest,published:published.length?Math.min(...published):null,views:top.views,likes:top.likes,status:'Early candidate' as const,narrative:null,detail:`${group.authors.size} creators · ${[...group.platforms].join(' + ')} · ${group.rows.length} supporting posts · unconfirmed`,quality}];}).sort((a,b)=>(b.quality||0)-(a.quality||0)||b.observed-a.observed).slice(0,60);
}
...
export async function GET(request:Request){
  ...
  try{
    ...
    const browser=await db().prepare(`SELECT e.id,e.platform,e.author,e.url,e.content,e.published,e.first_seen,e.last_seen,e.provenance,(SELECT o.views FROM observations o WHERE o.owner=e.owner AND o.id=e.id ORDER BY o.observed DESC LIMIT 1) AS views,(SELECT o.likes FROM observations o WHERE o.owner=e.owner AND o.id=e.id ORDER BY o.observed DESC LIMIT 1) AS likes FROM evidence e WHERE e.owner=? AND e.last_seen>? ORDER BY e.last_seen DESC LIMIT 4000`).bind(user.userId,now-14*86400000).all<BrowserRow>();
