...
const UI_NOISE = /^(show|show more|more|see more|view|view more|read more|explore|home|for you|trending|trend|news|sports|entertainment|posts?|replies?|likes?|views?|share|bookmark|follow|following|hashtag|hashtags|caption|video|videos|photo|photos|sound|original sound|user|username|creator|account|profile)$/i;
...
type CandidateLabel={key:string;label:string;kind:'hashtag'|'name'|'token'|'short'};
...
function slugTopic(url:string){
  try{
    const u=new URL(url);const parts=u.pathname.split('/').filter(Boolean);const index=parts.findIndex((part)=>part==='hashtag'||part==='trends');if(index>=0&&parts[index+1]){const decoded=decodeURIComponent(parts[index+1]).replace(/[-_]+/g,' ').trim();return UI_NOISE.test(decoded)||NUMERIC_ONLY.test(decoded)?'':decoded;}
  }catch{}
  return '';
}
function isJunkLabel(value:string,author=''){
  const clean=cleanCandidate(value);if(!clean||clean.length<2||UI_NOISE.test(clean)||SOCIAL_NOTIFICATION.test(clean)||NUMERIC_ONLY.test(clean))return true;
  if(author&&authorLike(clean,author))return true;
  if(/^@?[A-Za-z0-9_.]{2,40}$/.test(clean)&&author&&authorLike(clean,author))return true;
  return false;
}
function titleFor(row: BrowserRow) {
  if (SOCIAL_NOTIFICATION.test(row.content)) return '';
  if(/TikTok Creative Center/i.test(row.provenance)){const slug=slugTopic(row.url);if(slug)return slug;}
  const content = row.content.trim();
  const chunks = content.split(/\s+·\s+|\n+/).map(cleanCandidate).filter(Boolean);
  const hash=content.match(/#([\p{L}\p{N}_]{3,80})/u)?.[1]?.replace(/([a-z\d])([A-Z])/g,'$1 $2').replace(/_/g,' ');
  if(hash&&!isJunkLabel(hash,row.author))return hash.slice(0,180);
  const proper=content.match(/\b[A-Z][\p{L}\p{N}'’_-]{2,30}(?:\s+[A-Z][\p{L}\p{N}'’_-]{2,30}){0,2}\b/u)?.[0];
  if(proper&&!isJunkLabel(proper,row.author))return proper.slice(0,180);
  const meaningful = chunks.find((part) => !isJunkLabel(part,row.author) && !/^trending in\b/i.test(part) && !/^[\d,.]+\s*(?:posts?|views?|likes?)$/i.test(part));
  const candidate = meaningful || cleanCandidate(content);
  if (isJunkLabel(candidate,row.author)) return '';
  return candidate.slice(0, 180);
}

function signalQuality(item: PublicItem) { return !isJunkLabel(item.title); }
...
function balanced(items: PublicItem[]) {
  ...
  for (const item of items) { const list = buckets.get(item.sourceKey) ?? []; list.push(item); buckets.set(item.sourceKey, list); }
  ...
  let added = true;
  while (added) { added = false; for (const key of orderedKeys) { const item = buckets.get(key)?.shift(); if (!item) continue; result.push(item); added = true; } }
  return result;
}
...
function candidateLabels(row:BrowserRow):CandidateLabel[] {
  ...
  const add = (raw:string,kind:CandidateLabel['kind']) => {
    const label = cleanCandidate(raw).slice(0,80); const key = normalize(label); const tokens = candidateTokens(label);
    if (!key || key.length < 3 || isJunkLabel(label,row.author) || !tokens.length) return;
    if (tokens.every((token) => BROAD.has(token) || GENERIC.has(token))) return;
    if (!out.has(key)) out.set(key,{key,label,kind});
  };
  for (const match of row.content.matchAll(/#([\p{L}\p{N}_]{3,80})/gu)) add(match[1].replace(/([a-z\d])([A-Z])/g,'$1 $2').replace(/_/g,' '),'hashtag');
  for (const match of row.content.matchAll(/\b[A-Z][\p{L}\p{N}'’_-]{2,30}(?:\s+[A-Z][\p{L}\p{N}'’_-]{2,30}){0,2}\b/gu)) add(match[0],'name');
  for(const token of normalize(row.content).split(' ')) if(token.length>=4&&token.length<=30&&!STOP.has(token)&&!GENERIC.has(token)&&!BROAD.has(token)&&!/\d/.test(token)) add(token,'token');
  const title = titleFor(row); if (title && title.split(/\s+/).length <= 5) add(title,'short');
  return [...out.values()];
}
function buildEarlyCandidates(rows:BrowserRow[], linked:Set<string>, now:number):PublicItem[] {
  ...
  for (const row of rows) {
    ...
    for (const {key,label,kind} of candidateLabels(row)) {
      const group:CandidateGroup = groups.get(key) ?? {labels:new Map<string,number>(),rows:[],authors:new Set<string>(),platforms:new Set<string>(),kinds:new Set<CandidateLabel['kind']>()};
      if (!group.rows.some((item)=>item.id===row.id)) group.rows.push(row);
      group.authors.add(`${row.platform}:${row.author.toLowerCase()}`); group.platforms.add(row.platform);group.kinds.add(kind);
  ...
  return [...groups.entries()].flatMap(([key,group]) => {
    ...
    const structured=group.kinds.has('hashtag')||group.kinds.has('name');
    const crossPlatform=group.platforms.size>1;
    const hotSingle = structured&&group.rows.length===1 && ((top.views||0)>=100000 || (top.likes||0)>=5000);
    const enough = structured ? group.authors.size>=2||hotSingle : group.authors.size>=3||crossPlatform&&group.authors.size>=2;
    if (!enough) return [];
    const title = [...group.labels.entries()].sort((a,b)=>b[1]-a[1]||b[0].length-a[0].length)[0]?.[0] || key;
    if(isJunkLabel(title))return[];
  ...
  }).sort((a,b)=>b.observed-a.observed).slice(0,60);
}
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const browserItems: PublicItem[] = usableRows.flatMap((row) => {
      const meta = sourceFor(row), linked = linkByEvidence.get(row.id), title = titleFor(row);
      if (!title) return [];
      return [{id:`browser:${row.id}`,source:meta.source,sourceKey:meta.key,title,query:title,url:row.url,observed:row.last_seen,published:row.published,views:row.views,likes:row.likes,status:linked?'Promoted':'Raw signal',narrative:linked?{id:linked.narrative,title:linked.title}:null,detail:linked?`Promoted to ${linked.title}`:`${row.author} · observed from ${meta.source}`}];
    });
    ...
    try { external = await publicSignals(); } catch {}
    const externalItems: PublicItem[] = external.signals.map((signal) => ({id:`external:${signal.id}`,source:signal.source,sourceKey:signal.source==='Google Trends'?'google-trends':'market-theme',title:signal.title,query:signal.query,url:signal.url,observed:external.at,published:null,views:null,likes:null,status:'Raw signal',narrative:null,detail:signal.detail}));

    const allItems = dedupe([...earlyCandidates,...browserItems,...externalItems]);
    const counts:Record<string,number>={all:allItems.length};for(const item of allItems)counts[item.sourceKey]=(counts[item.sourceKey]||0)+1;
    ...
    const total=items.length,uniqueAuthors=new Set(usableRows.map((row)=>`${row.platform}:${row.author.toLowerCase()}`)).size,latestBrowserSeen=usableRows[0]?.last_seen??null;
    const ignoredNotifications=browser.results.filter((row)=>SOCIAL_NOTIFICATION.test(row.content)).length;
    const ignoredJunk=browser.results.length-usableRows.length-ignoredNotifications;
    const diagnosis=browser.results.length===0?'No synced browser evidence is stored yet. Run a fresh Browser Sources scan, or use Sync background finds if the local bridge has pending evidence.':usableRows.length===0?'Stored browser evidence is UI/metric noise rather than actual post content. Run a fresh scan after updating and restarting the bridge.':allItems.length===0?'Browser evidence exists, but none survived signal-quality parsing. This should be investigated as an extractor regression.':null;
    return json({items:items.slice(offset,offset+limit),page:{limit,offset,total,hasMore:offset+limit<total,nextOffset:offset+limit<total?offset+limit:null},counts,coverage:external.coverage,diagnostics:{browserEvidence:browser.results.length,usableBrowserEvidence:usableRows.length,ignoredNotifications,ignoredJunk,uniqueAuthors,earlyCandidates:earlyCandidates.length,sourceCounts,latestBrowserSeen,externalSignals:externalItems.length,diagnosis},at:now,note:'Public Signals now suppresses numbers, taxonomy labels, usernames and interface chrome. Repeated lowercase entities can become Early Candidates after independent creator corroboration, improving recall for names such as Astra without letting random one-off words flood the feed.'});
