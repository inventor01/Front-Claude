...
function lowValueSocialTopic(value:string){const clean=normalizeLoose(value);if(!clean)return true;const compact=compactLoose(value);if(SOCIAL_TAG_ONLY.has(compact))return true;const parts=clean.split(' ').filter(Boolean);return Boolean(parts.length)&&parts.every((part)=>SOCIAL_TAG_ONLY.has(part)||/^(?:meme|memes|viral|trend|trending|tiktok|twitter|video|videos|clip|clips|reaction|reactions|funny|social|media)$/i.test(part));}
...
function inferredTopics(value:unknown){
 ...
 for(const raw of value.slice(0,50)){
  ...
  const platforms=Array.isArray(row.platforms)?row.platforms.filter((x):x is string=>x==='X'||x==='TikTok').slice(0,2):[];const score=Number(row.score);const momentum=row.momentum&&typeof row.momentum==='object'?row.momentum as MomentumInput:null;const origin=row.originCandidate&&typeof row.originCandidate==='object'?row.originCandidate as OriginInput:null;
