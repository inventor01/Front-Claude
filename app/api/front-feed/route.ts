...
type Evidence={evidence_id?:string;platform:string;author:string;url:string;content:string;published:number|null;first_seen:number;views?:number|null;likes?:number|null;viewsPerMinute?:number|null;likesPerMinute?:number|null};
...
type Feed={cards:Card[];coins:CoinRow[];page?:{hasMore:boolean;nextOffset:number|null}};
...
type ObservationRow={id:string;observed:number;views:number|null;likes:number|null};
...
type OutputCoin={createdAt?:number|null;mint:string;name:string;symbol:string;matchType:string;matchScore:number;matchReason:string;marketCap:number|null;liquidity:number|null;volume24h:number|null;price:number|null;seen:number;verifiedPumpfun:true};
...
function postVelocity(row:Evidence,now:number):HotPost{
 ...
 const snapshotViews=numericOrNull(row.viewsPerMinute),snapshotLikes=numericOrNull(row.likesPerMinute),derivedViews=views!=null&&ageHours!=null?views/ageHours:0,observedViews=snapshotViews!=null?snapshotViews*60:0,derivedLikes=likes!=null&&ageHours!=null?likes/ageHours:0,observedLikes=snapshotLikes!=null?snapshotLikes*60:0;const viewsPerHour=Math.max(derivedViews,observedViews),likesPerHour=Math.max(derivedLikes,observedLikes);
...
async function observationVelocity(owner:string,now:number){
 const recent=now-24*3600000;let rows:ObservationRow[]=[];try{rows=(await db().prepare('SELECT id,observed,views,likes FROM observations WHERE owner=? AND observed>=? ORDER BY observed DESC LIMIT 8000').bind(owner,recent).all<ObservationRow>()).results;}catch{return new Map<string,{viewsPerMinute:number|null;likesPerMinute:number|null}>();}
 const pair=new Map<string,ObservationRow[]>();for(const row of rows){const list=pair.get(row.id)||[];if(list.length<2){list.push(row);pair.set(row.id,list);}}
 const out=new Map<string,{viewsPerMinute:number|null;likesPerMinute:number|null}>();for(const[id,list]of pair){if(list.length<2)continue;const latest=list[0],previous=list[1],elapsed=latest.observed-previous.observed;if(elapsed<60000||elapsed>24*3600000)continue;const minutes=elapsed/60000,latestViews=numericOrNull(latest.views),previousViews=numericOrNull(previous.views),latestLikes=numericOrNull(latest.likes),previousLikes=numericOrNull(previous.likes);const viewsPerMinute=latestViews!=null&&previousViews!=null&&latestViews>=previousViews?(latestViews-previousViews)/minutes:null;const likesPerMinute=latestLikes!=null&&previousLikes!=null&&latestLikes>=previousLikes?(latestLikes-previousLikes)/minutes:null;if(viewsPerMinute!=null||likesPerMinute!=null)out.set(id,{viewsPerMinute,likesPerMinute});}
 return out;
}
...
async function recordVerifiedOutcomes(owner:string,rows:OutputRow[],now:number){
 ...
 for(const row of rows.slice(0,50)){
  ...
  for(const coin of row.coins){
   if(!['exact','strong'].includes(coin.matchType)||coin.createdAt==null||coin.createdAt<detectedAt)continue;
...
async function buildFeed(owner:string,options:{recordOutcomes?:boolean}={}){
 const now=Date.now();
 const [stored,learned,observedVelocity]=await Promise.all([narrativeFeed(db(),owner,{limit:200,offset:0}) as unknown as Promise<Feed>,learningState(owner),observationVelocity(owner,now)]);
 ...
 const launchByKey=new Map(launches.filter((x)=>x.narrative).map((x)=>[`${x.narrative}:${x.mint}`,x]));const coins=(stored.coins||[]).filter((coin)=>(launchByKey.has(`${coin.narrative}:${coin.mint}`)||coin.data?.verificationSource==='DEX Screener direct Pump.fun pool')&&coin.data?.verifiedPumpfun===true);const coinsByNarrative=new Map<string,CoinRow[]>();for(const coin of coins){const list=coinsByNarrative.get(coin.narrative)||[];list.push(coin);coinsByNarrative.set(coin.narrative,list);}
...
const coinRefreshOffsets=new Map<string,number>();
