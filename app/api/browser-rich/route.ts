...
const feedSurface=(provenance:string)=>/X For You/i.test(provenance)?'x-for-you':/TikTok For You/i.test(provenance)?'tiktok-for-you':/sentinel/i.test(provenance)?'x-sentinel':/candidate investigation/i.test(provenance)?'candidate-investigation':/trend/i.test(provenance)?'trend-seed':null;

type RichEvidence={
 id?:unknown;platform?:unknown;provenance?:unknown;replies?:unknown;reposts?:unknown;bookmarks?:unknown;quotes?:unknown;comments?:unknown;shares?:unknown;saves?:unknown;soundId?:unknown;soundTitle?:unknown;soundAuthor?:unknown;mediaType?:unknown;quotedUrl?:unknown;coverUrl?:unknown;hashtags?:unknown;
};
type RichTopic={key?:unknown;topic?:unknown;feedPenetration?:unknown;feedPenetrationDelta?:unknown;feedPenetrationVelocity?:unknown;aliases?:unknown;soundSignals?:unknown};
...
export async function POST(r:Request){
 ...
 try{
  ...
  for(const item of evidence){
   ...
   evidenceStatements.push(db().prepare(`INSERT INTO evidence_rich(owner,id,observed,replies,reposts,bookmarks,quotes,comments,shares,saves,sound_id,sound_title,sound_author,media_type,quoted_url,cover_url,hashtags,feed_surface)
    VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
    ...
    .bind(user.userId,id,observed,metric(item.replies),metric(item.reposts),metric(item.bookmarks),metric(item.quotes),metric(item.comments),metric(item.shares),metric(item.saves),clean(item.soundId,120)||null,clean(item.soundTitle,240)||null,clean(item.soundAuthor,160)||null,clean(item.mediaType,30)||null,safeUrl(item.quotedUrl),safeUrl(item.coverUrl),hashtags,feedSurface(provenance)));
  }
  const topicStatements:D1PreparedStatement[]=[];
  for(const topic of topics){
   const key=clean(topic.key||topic.topic,180);
   if(!key)continue;
   const aliases=JSON.stringify(safeArray(topic.aliases,18,100));
   ...
   topicStatements.push(db().prepare(`INSERT INTO topic_rich_snapshots(owner,topic_key,observed,feed_penetration,feed_penetration_delta,feed_penetration_velocity,aliases,sound_signals)
    VALUES(?,?,?,?,?,?,?,?)
    ON CONFLICT(owner,topic_key,observed) DO UPDATE SET feed_penetration=excluded.feed_penetration,feed_penetration_delta=excluded.feed_penetration_delta,feed_penetration_velocity=excluded.feed_penetration_velocity,aliases=excluded.aliases,sound_signals=excluded.sound_signals`)
    .bind(user.userId,key,observed,finite(topic.feedPenetration),finite(topic.feedPenetrationDelta),finite(topic.feedPenetrationVelocity),aliases,JSON.stringify(sounds)));
  }
